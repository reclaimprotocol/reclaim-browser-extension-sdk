...
if (typeof global.require !== "function") {
  ...
  global.require = (moduleName) => {
    ...
    try {
      ...
      console.warn(`[POLYFILLS] Failed to require module: ${moduleName}`);
      return {};
    }
  };
}
...
if (!isServiceWorker) {
  ...
  try {
    ...
    console.warn("[POLYFILLS] Could not copy all URL static properties:", e);
  }
  ...
  try {
    ...
    console.warn("[POLYFILLS] Could not override URL constructor:", e);
...
global.fs = global.fs || {
  readFileSync: (path) => {
    console.warn(`[POLYFILLS] Attempted to readFileSync: ${path}`);
    throw new Error("fs.readFileSync is not available in this environment");
  },
  writeFileSync: (path, data) => {
    console.warn(`[POLYFILLS] Attempted to writeFileSync: ${path}`);
    // No-op
  },
  existsSync: (path) => {
    console.warn(`[POLYFILLS] Attempted to check if file exists: ${path}`);
    return false;
  },
  promises: {
    readFile: async (path) => {
      console.warn(`[POLYFILLS] Attempted to readFile: ${path}`);
      throw new Error("fs.promises.readFile is not available in this environment");
    },
    writeFile: async (path, data) => {
      console.warn(`[POLYFILLS] Attempted to writeFile: ${path}`);
      // No-op
    },
  },
};
